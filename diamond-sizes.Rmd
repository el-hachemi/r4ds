...
# I've put include=FALSE but with the global chunk options set with echo=FALSE then it'll not be in the report so I'll suppress only the messages that my ressort with message=FALSE, and puting the smaller in another chunk
library(ggplot2)
library(dplyr)
```
...
Diamonds with lower quality cuts (cuts are ranked from "Ideal" to "Fair") 
tend to be larger.
```{r}
...
The pattern present in cut and color os also present in clarity. Diamonds with 
...
This patterns are consistent with there being a profitability threshold for retail diamonds that is a function of carat, clarity, color, cut and other characteristics. A diamond may be profitable to sell if a poor value of one feature, for example, poor clarity, color or cut, is be offset by a good value 
of another feature, such as large size. This can be considered an example of [Berkson's paradox](https://en.wikipedia.org/wiki/Berkson%27s_paradox).  
...
The largest twenty diamonds (by carat) in the dataset are,
```{r results = "asis"}
diamonds %>%
  arrange(desc(carat)) %>%
  slice(1:20) %>%
