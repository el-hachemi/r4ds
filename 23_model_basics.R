...
model1(c(7, 1.5), sim1)
# [1]  8.5  8.5  8.5 10.0 10.0 10.0 11.5 11.5 11.5 13.0 13.0 13.0 14.5 14.5 14.5 # [16] 16.0 16.0 16.0 17.5 17.5 17.5 19.0 19.0 19.0 20.5 20.5 20.5 22.0 22.0 22.0
...
measure_distance(c(7, 1.5), sim1) # [1] 2.665212
...
ggplot(sim1a, aes(x, y)) + 
  ...
  geom_abline(intercept = coef[1], slope = coef[2])
      ...
      # When I rerun few times I noticed that the line tend to the side of the the outliers, and this is how jrnolds solution look like:
...
measure_distance <- function(mod, data) {
  ...
  mean(abs(diff))
      ...
      # For the above function to work, we need to define a function, model1(), that takes a numeric vector of length two (the intercept and slope) and returns the predictions,

model1 <- function(mod, data) {
  mod[1] + mod[2] * data$x
} # The first step is to write the model in a function

measure_distance_abs <- function(mod, data) {
  diff <- data$y - model1(mod, data)
  mean(abs(diff))
} # Second calculate the distance with a method here is the mean of the abs

      # Using the sim1a data, the best parameters of the least absolute deviation are:

best_abs <- optim(c(0, 0), measure_distance, data = sim1a)
best_abs$par # [1] 5.871393 1.517246

      # To compare with the linear I'll calculate with the square version

measure_distance_sq <- function(mod, data) {
  diff <- data$y - model1(mod, data)
  sqrt(mean(diff ^ 2))
}
  
best_sq <- optim(c(0, 0), measure_distance_sq, data = sim1a)
best_sq$par # 5.234897 1.636310
  
ggplot(sim1a, aes(x, y)) +
  geom_point() +
  geom_abline(intercept = best_abs$par[1], slope = best_abs$par[2], color = "blue") +
  geom_abline(intercept = best_sq$par[1], slope = best_sq$par[2], color = "red")
...
best3a <- optim(c(0, 0, 0), measure_distance3, data = sim1)
best3a$par # [1] 3.3672228 2.0515737 0.8528513

best3b <- optim(c(0, 0, 1), measure_distance3, data = sim1)
best3b$par # [1] -3.469885  2.051509  7.690289
...
ggplot(sim1, aes(x, resid)) +
  ...
  #~~~~~~~~~~~~~~~~
    ...
    # > 1. Instead of using lm() to fit a straight line, you can use loess() to fit a smooth curve. Repeat the process of model fitting, grid generation, predictions, and visualisation on sim1 using loess() instead of lm(). How does the result compare to geom_smooth()?
      ...
      # First the model fitting called sim1_mod_loess

sim1_mod <- lm(y ~ x, data = sim1)
sim1_mod_loess <- loess(y ~ x, data = sim1)
sim1_mod_loess

      # Now grid generation, predictions and visualisation

grid <- sim1 %>% data_grid(x) # The generation is the same as sim1_mod with lm
grid_loess <- grid %>% add_predictions(sim1_mod_loess) # predictions
grid_loess
...
?add_predictions # From the help page we can see that: add_prediction adds a single new column, with default name pred, to the input data. spread_predictions adds one column for each model. gather_predictions adds two columns .model and .pred, and repeats the input rows for each model.

      # let's test them with the two models that we have above:

grid %>% # adds only a single model at a time, with default name pred
  add_predictions(sim1_mod, var = "pred_lm") %>% 
  add_predictions(sim1_mod_loess, var = "pred_loess")

grid %>% # adds two columns .model and .pred, and repeats the input rows for each model.
  gather_predictions(sim1_mod, sim1_mod_loess)

grid %>% # adds one column for each model.
  spread_predictions(sim1_mod, sim1_mod_loess)

    # > 3. What does geom_ref_line() do? What package does it come from? Why is displaying a reference line in plots showing residuals useful and important?

      # The geom_ref_line() add a reference line in the ggplot2 package, it can be horizontal or vertical she can take a size which is 2 by default and a color which is "white" by default, it come from the modelr package. 
...
ggplot(sim1, aes(x = abs(resid))) +
  geom_freqpoly(binwidth = 0.5)
      ...
      # Here we can see the two frequency polygon, one for the absolute and one for the raw residuals
ggplot(sim1) +
  geom_freqpoly(aes(x = abs(resid)), binwidth = 0.5) +
  geom_freqpoly(aes(x = resid), binwidth = 0.5, color = "red")
...
x2 <- c(0, 1)
seq_range(x2, n = 5) # [1] 0.00 0.25 0.50 0.75 1.00
seq_range(x2, n = 5, expand = 0.10) # [1] -0.050  0.225  0.500  0.775  1.050
seq_range(x2, n = 5, expand = 0.25) # [1] -0.1250 0.1875 0.5000 0.8125 1.1250
seq_range(x2, n = 5, expand = 0.50) # [1] -0.250  0.125  0.500  0.875  1.250
...
grid <- sim2 %>% # Takin a dataset
  data_grid(x) %>% # making a grid with the x's
  add_predictions(mod2) # adding to x's the model's predictions
grid

# Visualising the predictons

# For continuous variables, We got an error message
ggplot(sim2, aes(x)) +
  geom_point(aes(y)) + # When ajusting aes(y = y) we got the plot without the geom line and with a message geom_path: Each group consists of only one observation. Do you need to adjust the group aesthetic?
...
ggplot(sim2, aes(x)) +
  geom_point(aes(y = y)) +
  geom_point(aes(y = pred), data = grid, color = "red", size = 4)


# Now I'll repeat the same analysis without intercept, to do so I will add -1 to right hand of the model

mod2a <- lm(y ~ x -1, data = sim2)

# To see what happend

model_matrix(sim2, mod2a)
model_matrix(sim2, y ~ x -1)

model_matrix(sim2, mod2)
model_matrix(sim2, y ~ x)

# We have the same thing except for the first column that is full of ones which change the name from intercept to xa; let's see what happened to the predictions

grid_a <- sim2 %>% 
  data_grid(x) %>% 
  add_predictions(mod2a)
grid_a # The grid_a is identical to grid 

# Because grid is identical to grid_a so the visualisation will be the same

ggplot(sim2, aes(x = x)) +
  geom_point(aes(y = y)) +
  geom_point(aes(y = pred), data = grid_a, color = "red", size = 5)


    # > 2. Use model_matrix() to explore the equations generated for the models I fit to sim3 and sim4. Why is * a good shorthand for interaction?

      #model_matrix(sim3, y ~ x1 + x2)
mod2_sim3 <- lm(y ~ x1 * x2, sim3)
mod1_sim3 <- lm(y ~ x1 + x2, sim3)

# For x1 * x2 when x2 is a categorical variable produces indicator variables x2b, x2c, x2d and variables x1:x2b, x1:x2c and x1:x2d which are the products of x1 an x2* variables:

model_matrix(sim3, y ~ x1 * x2)

# We can confirm that the variables x1:x2b is the product of x1 and x2b,

all(mod2_sim3[["x1:x2b"]] == (mod2_sim3[["x1"]] * mod2_sim3[["x2b"]]))
# or
stopifnot(mod2_sim3[["x1:x2b"]] == (mod2_sim3[["x1"]] * mod2_sim3[["x2b"]]))

# For x1 * x2 where both x1 and x2 are continuous variables, model_matrix() creates variables x1, x2, and x1:x2:

mod2_sim4 <- lm(y ~ x1 * x2, sim4)
mod1_sim4 <- lm(y ~ x1 + x2, sim4)

md4 <- model_matrix(sim4, mod2_sim4)

# confirm that x1:x2 is the product of the x1 and x2

all(md4$x1 * md4$x2 == md4$`x1:x2`)
#or 
all(md4[["x1"]] * md4[["x2"]] == md4[["x1:x2"]]) # This type of verification always returned TRUE for me even if I change the variables or I delete them, the first one is sensible for changements.
...
model_matrix_mod1b <- function(.data) {
  lvls <- levels(.data$x2) # the levels of x2
  lvls <- lvls[2:length(lvls)] # drop the first level 
  for (lvl in lvls) { # Create an indicator variable for each level of x2
    varname <- str_c("x2", lvl) # new column name x2 + level name
    .data[[varname]] <- as.numeric(.data$x2 == lvl) # add indicator variable for lvl
  }
  X2_variables <- str_c("x2", lvls) # generate the list of variables to keep
...
model_matrix_mod2b <- function(.data) {
  out <- model_matrix_mod1b(.data) #get dataset with x1,x2 indicator variables
  x2col <- str_subset(colnames(out), "^x2") # get names of the x2 indicator col
  # Create interactions between x1 and x2 indicator columns
  for (varname in x2col) {
    newvar <- str_c("x1:", varname) # name of the interaction variable
...
#> 2 mod2   2.07
      ...
      # The standard deviation of the residuals of mod2 os smaller than that os mod1.
